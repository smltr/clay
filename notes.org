...
* New ideas on 6/29/2025

    So I'm thinking to change up a bit how indentation works to allow for better syntax

    instead of a dedent automatically being a close out, we will use a do and end keyword set

    also, anything inside an indent will be treated as a block or list instead of individual items, and anything on the same indentation but before an end will be a continuation
    ...
    ```lisp
    (if (odd num) ((print "odd"))
        (if (even num) ((print "even"))
        (else (print "neither))
        )
    )
    ```
    
    do captures everything into a block (list) until the next dedented line
